...
| `tools/spec.test.mjs` is a single growing check file | Cycle 0 had no packages | M1 remainder creates `packages/schemas`; move schema validation there |
| Cycle 1 ZIP uses STORE, not deflate | Smallest deterministic archive | M4 per GENERATION_CONTRACT.md |
| Compile API is unauthenticated | Cycle 1 local demo | M2 |
